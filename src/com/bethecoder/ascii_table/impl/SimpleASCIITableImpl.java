...
/**
 * This implementation builds the header and data rows by considering each column one by one in the format of 'FFFF...' (border format). While
 * rendering the last column, it closes the table. It takes <max_length_string_in_each_column>+3 characters to render each column. These three
 * characters include two white spaces at left and right side of the max length string in the column and one char(|) for table formatting.
 * 
...
public class SimpleASCIITableImpl implements IASCIITable {
    ...
    public String getTable( ASCIITableHeader[] headerObjs, String[][] data ) {
        ...
        if ( header != null && header.length > 0 ) {
            ...
            tableBuf.append( getRowLineBufHeader( colCount, colMaxLenList, data ) );
    ...
    private String getRowLineBufHeader( int colCount, List<Integer> colMaxLenList, String[][] data ) {
